...
from database import fetch_one_pokemon, fetch_all_pokemon, create_pokemon, update_pokemon, delete_pokemon_from_db, upsert_pokemon_db, delete_pokemon_from_db, reload_pokemon_db
...
@app.post("/api/v1/pokemon/reload")
async def reload_pokemon():
    try:
        result = await reload_pokemon_db()
        if result:
            return {'message': f'{result}'}
        raise HTTPException(status_code=500, detail="Failed to reload the Pokémon")
...
async def delete_pokemon(p_id: int):
    ...
    try:
        ...
        if response:
            return {'message': 'Database update successful'}, 200
        raise HTTPException(status_code=404, detail=f"Pokemon with ID {p_id} not found")
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
